import { union, difference, intersection, isEqual, trim } from 'lodash-es'
...
const LENGTH_EASY_DIGITS = [2, 3, 4, 7]
const DECODE_DIGIT_0 = [0, 1, 2, 4, 5, 6]
const DECODE_DIGIT_1 = [2, 5]
const DECODE_DIGIT_2 = [0, 2, 3, 4, 6]
const DECODE_DIGIT_3 = [0, 2, 3, 5, 6]
const DECODE_DIGIT_4 = [1, 2, 3, 5]
const DECODE_DIGIT_5 = [0, 1, 3, 5, 6]
const DECODE_DIGIT_6 = [0, 1, 3, 4, 5, 6]
const DECODE_DIGIT_7 = [0, 2, 5]
const DECODE_DIGIT_8 = [0, 1, 2, 3, 4, 5, 6]
const DECODE_DIGIT_9 = [0, 1, 2, 3, 5, 6]
const DECODE_DIGIT = [
  DECODE_DIGIT_0,
  DECODE_DIGIT_1,
  DECODE_DIGIT_2,
...
const formatData = input => {
  const formattedInput = input.map(element => element.split(' | ').map(e => trim(e)))
  formattedInput.forEach((line, index, array) => {
    array[index][0] = line[0].split(' ')
    array[index][1] = line[1].split(' ')
  })

  return formattedInput
}

const countEasyDigit = outputDigits => {
  let count = 0
  outputDigits.forEach(digit => {
    if (LENGTH_EASY_DIGITS.includes(digit.length)) {
      count += 1
    }
  })
  return count
...
const generateTranslator = inputDigits => {
  let translation = Array(7).fill(0)
  inputDigits.sort((a, b) => a.length - b.length)
  const digitOne = inputDigits[0]
  const digitSeven = inputDigits[1]
  const digitFour = inputDigits[2]
  const topDigitFive = difference(union([...digitFour], [...digitSeven]), [...digitOne])

  // Find top
  translation[0] = difference([...digitSeven], [...digitOne])[0]
  ...
  inputDigits
    ...
    .forEach(digit => {
      differences.push(difference([...digit], topDigitFive))
    })

  const differencesIntersection = intersection(differences[0], differences[1], differences[2])
  translation[6] = differencesIntersection[0]
  ...
  inputDigits
    .filter(digit => digit.length === 5)
    .forEach(digit => {
      const bottomRight = difference(
        difference(difference([...digit], topDigitFive), [...digitOne]),
        [translation[6]],
      )

      if (bottomRight.length === 1) {
        translation[4] = bottomRight[0]
        digitTwo = digit
      }
    })

  // Top right
  translation[2] = intersection([...digitTwo], [...digitOne])[0]

  // Bottom Left
  translation[5] = difference([...digitOne], [translation[2]])[0]

  // Top Left
  translation[1] = difference(
    ['a', 'b', 'c', 'd', 'e', 'f', 'g'],
    union([...digitTwo], [...digitOne]),
  )[0]

  // Find Last
  const lastChar = difference(
    ['a', 'b', 'c', 'd', 'e', 'f', 'g'],
    translation.filter(t => t !== 0),
  )
  const lastIndex = translation.findIndex(t => t === 0)

  translation[lastIndex] = lastChar[0]

  return translation
}

const decodeOutputDigits = (translation, outputDigits) => {
  let numberStr = ''
  let outputs = []
  ...
  outputDigits.forEach(digit => {
    ...
    arrDigit.forEach(char => {
      output.push(translation.findIndex(e => e === char))
    })
    outputs.push(output)
  })
  ...
  outputs.forEach(output => {
    ...
    DECODE_DIGIT.forEach((digit, index) => {
      if (isEqual(digit, output)) {
...
const firstSolution = signalWires => {
  let totalCount = 0
  signalWires.forEach(line => {
    totalCount += countEasyDigit(line[1])
  })

  return totalCount
}

const secondSolution = signalWires => {
  let total = 0

  signalWires.forEach(line => {
    const translation = generateTranslator(line[0])
    const numberOutput = decodeOutputDigits(translation, line[1])
    total += numberOutput
  })
...
const input = `fdceba bafdgc abeg afbdgec gbeacd abced bgc fcdge bg bedgc | bafdec cgefd gcebd ebcgd
  ...
  gcedb fdcb aebgcd bf fcgabe cfbaedg fdbceg bfegd aedgf gbf | bfg cdgeb fdebg cebdg`.split('\n')

const signalWires = formatData(input)

console.log({ firstSolution: firstSolution(signalWires) })
console.log({ secondSolution: secondSolution(signalWires) })
